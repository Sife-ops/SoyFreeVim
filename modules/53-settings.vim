...
if filereadable(expand(glob(g:sife_init_d."/*-nnn.vim")))
    let g:nnn#layout = { 'window': { 'width': 0.9, 'height': 0.9, 'border': 'sharp' } }
endif

if filereadable(expand(glob(g:sife_init_d."/*-fzf.vim")))
    let g:fzf_layout = { 'window': { 'width': 0.9, 'height': 0.9, 'border': 'sharp' } }
endif

if filereadable(expand(glob(g:sife_init_d."/*-vim-easymotion.vim")))
    map <leader><leader>j <Plug>(easymotion-overwin-line)
    map <leader><leader>k <Plug>(easymotion-overwin-line)
    map <leader><leader>w <Plug>(easymotion-overwin-w)
endif

if filereadable(expand(glob(g:sife_init_d."/*-fern.vim")))
...
if filereadable(expand(glob(g:sife_init_d."/*-vim-floaterm.vim")))
    nnoremap <leader>f :FloatermNew lf<cr>
    nnoremap <leader>z :FloatermNew<cr>
    " nnoremap <leader>z :botright split term://zsh<cr>
endif

if filereadable(expand(glob(g:sife_init_d."/*-gruvbox.vim")))
    colorscheme gruvbox
    set background=dark
    hi Normal guibg=0
endif

if filereadable(expand(glob(g:sife_init_d."/*-vim-hexokinase.vim")))
    let g:Hexokinase_highlighters = ['backgroundfull']
endif

if filereadable(expand(glob(g:sife_init_d."/*-better-vim-tmux-resizer.vim")))
...
if filereadable(expand(glob(g:sife_init_d."/*-undotree.vim")))
    nnoremap <leader>u :UndotreeShow<cr>
endif

if filereadable(expand(glob(g:sife_init_d."/*-vimwiki.vim")))
